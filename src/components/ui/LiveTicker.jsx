...
const CRYPTO_SYMBOLS = ['BINANCE:BTCUSDT','BINANCE:ETHUSDT','BINANCE:BNBUSDT','BINANCE:SOLUSDT'];

export default function LiveTicker() {
  const quotes = useMarketData();
  const [active, setActive] = useState('stocks');

  const syms = active === 'stocks' ? STOCK_SYMBOLS : CRYPTO_SYMBOLS;
  const names = {
    'BINANCE:BTCUSDT':'BTC/USD',
    'BINANCE:ETHUSDT':'ETH/USD',
    'BINANCE:BNBUSDT':'BNB/USD',
    'BINANCE:SOLUSDT':'SOL/USD',
    'AAPL':'Apple','MSFT':'Microsoft','SPY':'S&P 500','QQQ':'NASDAQ',
  };
          ...
          {data.map((d,i)=>(
            <div key={i} className="rounded-xl bg-[var(--bg-card)] border border-[var(--border)] p-3.5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-bold text-[var(--text)]">{names[d.s]||d.s}</span>
